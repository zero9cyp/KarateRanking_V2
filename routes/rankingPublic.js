...
// helper για age & weight categories
...
function getSeasons() {
  ...
  return years.length ? years.map(y => y.year) : [2025, 2024];
}
...
router.get("/view", (req, res) => {
  ...
  let sql = `
    SELECT 
      ...
      (
        SELECT GROUP_CONCAT(t.name || ' (' || r2.placement || 'η θέση)', ', ')
        FROM results r2
        JOIN tournaments t ON t.id = r2.tournament_id
    ...
    WHERE r.event_type = ?
      AND r.season_year = ?
  `;
  const params = [event_type, season, event_type, season];
  ...
  if (weight_id) {
    sql += " AND wc.id = ?";
    params.push(weight_id);
  }

  sql += " GROUP BY a.id ORDER BY total_points DESC, a.full_name ASC";
